...
def main():
    ...
    while True:
        ...
        if choice == 2:
            ...
            while True:
                try:
                    four_digits = int(input("Enter the last four digits of your card number: \n"))
                    if four_digits >= 10000:
                        raise ValueError
                    remaining_balance = float(input("Enter the remaining balance to 2 decimal places\n"))
                    if remaining_balance != round(remaining_balance, 2):
                        raise ValueError
                    cleared_by = str(input("Enter the date you wish to have this debt cleared by (DD/MM/YYYY): \n"))
        ...
        if choice == 3:
            ...
            while True:
                try:
                    ...
                    if four_digits >= 10000:
                        raise ValueError
                    amount_paid = float(input("Enter the amount paid to 2 decimal places\n"))
        ...
        if choice == 4:
            ...
            while True:
                try:
                    ...
                    if four_digits >= 10000:
                        raise ValueError
                    balance_added = float(input("Enter the amount to be added to balance to 2 decimal places\n"))
...
def view_extant_debts():
    with open("existing_debts.txt", "r") as file:
        ...
        table.field_names = ["Last Updated", "Card ending in ...", "Remaining Balance", "Avg Monthly Repayment", "No. Payments Remaining", "Debt cleared by"]
...
def add_new_cc(four_digits: int, remaining_balance: float, cleared_by:str) -> None:
    ...
    num_of_repayments = 12 * (cleared_by.year - today.year) + (cleared_by.month - today.month)
    ...
    with open("existing_debts.txt", "a") as file:
        entry = [today.strftime('%a %d %b %Y'), four_digits, f"£{remaining_balance:.2f}", f"£{avg_repayment:.2f}", num_of_repayments, cleared_by.strftime('%a %d %b %Y')]
